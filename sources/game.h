...
class Game
{
	public:
		...
		uint32_t getPlayersOnlineUnique();
		...
		/**
		  ...
		  */
		uint64_t getMoney(const Cylinder* cylinder, bool includeBank = true);
		...
		/**
		  ...
		  * \returns true if the removal was successful
		  */
		bool removeMoney(Cylinder* cylinder, int64_t money, uint32_t flags = 0, bool includeBank = true);
		...
		bool playerReceiveNewPing(uint32_t playerId, uint16_t ping, uint16_t fps);
		...
		void playerToggleMount(uint32_t playerId);
