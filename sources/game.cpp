	...
	if (direction < SOUTHWEST && creature->getPlayer())
	{
		...
		if (currentPos.z != 8 && creature->getTile()->hasHeight(3)) // try go up
		{
			if ((!(tmpTile = map->getTile(Position(currentPos.x, currentPos.y, currentPos.z - 1))) || (!tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID))) &&
				(tmpTile = map->getTile(Position(destPos.x, destPos.y, destPos.z - 1))) && tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID) && !tmpTile->hasProperty(FLOORCHANGEDOWN))
			{
						  ...
						  uint16_t spriteId, int16_t fromStackpos, const Position &toPos, uint8_t count)
{
	...
	if (!fromCylinder || !toCylinder || !item || (item->getClientID() != spriteId && item->getReplaceID() != spriteId))
	{
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
...
uint64_t Game::getMoney(const Cylinder *cylinder, bool includeBank /*= true*/)
{
	if (!cylinder)
		return 0;
...
bool Game::removeMoney(Cylinder *cylinder, int64_t money, uint32_t flags /*= 0*/, bool includeBank /*= true*/)
{
	if (!cylinder)
		return false;
...
bool Game::playerReceiveNewPing(uint32_t playerId, uint16_t ping, uint16_t fps)
{
	Player *player = getPlayerByID(playerId);
	if (!player || player->isRemoved())
	{
								 ...
								 uint32_t creatureId, uint16_t spriteId, bool isHotkey)
{
	...
	Item *item = thing->getItem();
	if (!item || (item->getClientID() != spriteId && item->getReplaceID() != spriteId))
	{
		player->sendCancelMessage(RET_CANNOTUSETHISOBJECT);
		return false;
	}
...
bool Game::playerRotateItem(uint32_t playerId, const Position &pos, int16_t stackpos, const uint16_t spriteId)
{
	...
	Item *item = thing->getItem();
	if (!item || (item->getClientID() != spriteId && item->getReplaceID() != spriteId) || !item->isRoteable() || (item->isLoadedFromMap() && (item->getUniqueId() || (item->getActionId() && item->getContainer()))))
	{
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
							  ...
							  uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	if (!tradeItem || (tradeItem->getClientID() != spriteId && tradeItem->getReplaceID() != spriteId) || !tradeItem->isPickupable() || (tradeItem->isLoadedFromMap() && (tradeItem->getUniqueId() || (tradeItem->getActionId() && tradeItem->getContainer()))))
	{
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
...
bool Game::playerLookAt(uint32_t playerId, const Position &pos, uint16_t spriteId, int16_t stackpos)
{
	...
	if (player->hasCustomFlag(PlayerCustomFlag_CanSeeCreatureDetails))
	{
		if (const Creature *creature = thing->getCreature())
		{
			...
			{
				ss << std::endl
				   << "IP: " << convertIPAddress(target->getIP());
...
bool Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
	...
	if (player->getTile()->hasFlag(TILESTATE_PROTECTIONZONE) && outfit.lookMount != 0 && !g_config.getBool(ConfigManager::ALLOW_PZ_MOUNTS))
	{
		outfit.lookMount = 0;
	}
...
bool Game::combatChangeHealth(const CombatParams &params, Creature *attacker, Creature *target, int32_t healthChange, bool force)
{
	...
	else
	{
		...
		if (damage > 0)
		{
			...
			Player *player = NULL;
			if ((player = target->getPlayer()) && (attacker->getPlayer()))
			{
				// config area
				int8_t randomize = rand() % 101;

				// dodge
				bool dodge = false;
				std::string storageDodge = "98798644";
				std::string dodgeV;
				int16_t dodgeVal = 0;

				if (player->getStorage(storageDodge, dodgeV))
				{
					dodgeVal = atoi(dodgeV.c_str());
					if (dodgeVal > 0)
					{
						if (randomize <= dodgeVal / 6)
						{
							dodge = true;
						}
					}
				...
				if (dodge == true)
				{
					...
					buffer << "DODGE" << std::endl;
					addAnimatedText(list, targetPos, 250, buffer.str());
					return false;
				}
							...
							CombatType_t combatType /* = COMBAT_MANADRAIN*/, bool inherited /* = false*/)
{
	...
	else
	{
		...
		if (manaLoss > 0)
		{
			Player *ply = NULL;
			SpectatorVec listt;
			getSpectators(listt, targetPos, true, true);
			if ((ply = target->getPlayer()) && (attacker->getPlayer()))
			{
				// config area
				int8_t randomize = rand() % 101;
				...
				if (ply->getStorage(storageDodge, dodgeV))
				{
					dodgeVal = atoi(dodgeV.c_str());
					if (dodgeVal > 0)
					{
				...
				if (dodge == true)
				{
					std::stringstream buffer;
					buffer << "DODGE" << std::endl;
					addAnimatedText(listt, targetPos, 250, buffer.str());
					return false;
				}
			}
			bool deny = false;
...
Position Game::getClosestFreeTile(Creature *creature, Position pos, bool extended /* = false*/, bool ignoreHouse /* = true*/)
{
	...
	if (Player *player = creature->getPlayer())
	{
		for (PairVector::iterator it = relList.begin(); it != relList.end(); ++it)
		{
			...
			ReturnValue ret = tile->__queryAdd(0, player, 1, FLAG_IGNOREBLOCKITEM);
			if (ret == RET_NOTENOUGHROOM || (ret == RET_NOTPOSSIBLE && !player->hasCustomFlag(PlayerCustomFlag_CanMoveAnywhere)) || (ret == RET_PLAYERISNOTINVITED && !ignoreHouse))
				continue;
...
uint32_t Game::getPlayersWithMcLimit()
{
	...
	for (AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
	{
		if (!it->second->isRemoved() && it->second->getIP() != 0 && it->second->getIdleTime() < 960000)
		{
...
uint32_t Game::getPlayersOnlineUnique()
{
	...
	for (AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
	{
		// nao conta quem deu exit, nao conta quem ta away a mais de 15m
		if (!it->second->isRemoved() && it->second->getIP() != 0 && it->second->getIdleTime() < 960000)
		{
			uint32_t ip = it->second->getIP();
			if (ips.find(ip) == ips.end())
...
void Game::playerToggleMount(uint32_t playerId)
{
	Player *player = getPlayerByID(playerId);
	if (!player || player->isRemoved())
	{
